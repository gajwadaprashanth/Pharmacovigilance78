...
global $Connection;
$db="SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES where TABLE_NAME='$tablename'";
echo $db;
$Execute=mysqli_query($Connection,$db);
if(mysqli_num_rows($Execute)>0){
	
	$url = 'createtable.php?name='.$na;
	?>
	<script>
	...
	echo "2";
$query="update subindex set tablename='$tablename' where name='$na'";
...
<form action="addcolumn.php?no=<?php echo $num; ?>&tablename=<?php echo $tablename; ?>&tablesub=<?php echo $tablesub; ?>&name=<?php echo $na; ?>" method="POST">
<?php
for ($i=0; $i<$num; $i++)
{   echo "Column ".($i+1);
	?>
	<input class ="form-control" placeholder = "Enter Column Names" type="text" name="anything[]" value=""><br>
	<?php
}
?>
<input type="submit" class="btn btn-success mt-2"name="submit" value="Add column">
</form>
<?php
}
}
else{?>
	 <form action="createtable.php?name=<?php echo $na;?>&tablesub=<?php echo $tablesub; ?>" method="POST">
					<label>Enter Table Name:</label><br>
                    <input class = "form-control" placeholder = "Table Name" type="text" name="tablename" value="">*<?php echo $error; ?><br>
                    <label>How Many columns Do You Want To Add:</label><br>
                    <input class = "form-control" placeholder = "Column Number" type="text" name="column" value="">
