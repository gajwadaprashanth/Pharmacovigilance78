...
$cnt=array();
 $cnt=$_POST['selection'];
 $cont=0;
 $suntable=$_GET['subtable'];
 foreach($cnt as $delid)
  {
     
    $cont++;
     $conn=mysqli_connect('localhost','root','','pv');
     $query= mysqli_query($conn,"DELETE FROM $suntable WHERE id='$delid'");
     if($query){?>
         <script>
...
for($i=0;$i<$s;$i++){
    ...
    $query="update `$suntable` set $name='$set' where id='$id'";
...
?>                          
	...
	$rows=$row[$a[$i]];				  
	 ?>
	<form action="edittable.php?name=<?php echo $na;?>&table=<?php echo $table; ?>&subtable=<?php echo $tablename; ?>" method="POST">
	 <td scope="row"><?php if($i==0){
           echo "<input type=\"checkbox\" name=\"selection[]\" value =".$row['id'].">". '<br/>'.'<center>';
        }else{}

			?></td>					
	<input type="hidden" name="name[]" value="<?php echo $a[$i];?>"> 
    <input type="hidden" name="number[]" value="<?php echo $row['id'];?>"> 	
	<td scope="row"><input type="text" name="input[]"value="<?php echo $rows; ?>"></td>
...
<?php }else{} ?>
             <input type="submit" style="margin-bottom:15px;" class="btn btn-warning" name="delete" value="Delete">
	<input type="submit" style="margin-bottom:15px;" class="btn btn-success" name="submit" value="Edit">
		</form>	
